...
class RedisSyncContainer(containers.DeclarativeContainer):
    ...
    redis_sync_uow = providers.Factory(
        redis_uow.SyncRedisUOW, redis_sync_repository
    )
...
class RedisAsyncContainer(containers.DeclarativeContainer):
    ...
    redis_async_uow = providers.Factory(
        redis_uow.AsyncRedisUOW, redis_async_repository
    )
